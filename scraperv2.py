...
DOWNLOAD_DIR = os.getenv("SDG_DOWNLOAD_DIR", os.path.abspath("downloads"))
...
def merge(identifier, analysis_type):
    ...
    authors = []
    journal_origin = []
    filepath = os.getenv("SDG_MERGE_INPUT_DIR", DOWNLOAD_DIR)
    dir_list = os.listdir(filepath)
    print(dir_list)
    print(len(dir_list))
    if not dir_list:
        raise RuntimeError(f"No exported CSV files found in {filepath}")
    if analysis_type == "journal":
        excelcsv = pd.read_csv(os.path.join(filepath, str(dir_list[0])))
    ...
    for i in range(len(dir_list)):
        excelcsv = pd.read_csv(os.path.join(filepath, str(dir_list[i])))
